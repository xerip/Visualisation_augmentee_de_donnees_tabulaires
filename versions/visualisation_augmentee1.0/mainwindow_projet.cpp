...
void MainWindow_projet::initialiser_couleurs_occuppees(int col)
{
...
QVector< QVector<QColor> > MainWindow_projet::extraire_couleurs()
{
    ...
    //int *couleurOccupees = new int[26][26][26];

    //QVector< QVector< QVector<int> > > couleurOccupees (26, 26, QVector<int>(26));
    ...
    {
        qDebug() << "\ncol " << col;
        initialiser_couleurs_occuppees(col);
        ...
        for (int row=0; row<_model->rowCount(); row++)
        {
            ...
            if ((r==0)&&(g==0)&&(b==0))
            {
                ...
                while (_couleurOccupees[r][g][b]);
                _couleurOccupees[r][g][b]=1;
...
int MainWindow_projet::colors_empty()
{
    for (int r=0; r<=25; r++) {
        for (int v=0; v<=25; v++) {
            for (int b=0; b<=25; b++) {
                if (!_couleurOccupees[r][v][b]) return 0;
            }
        }
    }
    return 1;
